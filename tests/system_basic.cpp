...
namespace {
    ...
    SystemDesc describeSystem(const Query& query, Func&& callback) {
        return {
            .query = query,
    ...
    SystemDesc markPhase(std::vector<Phase>& phases, const Phase phase) {
        return describeSystem(query<>(), [&phases, phase](const EcsVec<TableId>&, World&) {
...
Test(system, registered_query_picks_up_matching_tables_created_after_system_registration) {
    ...
    int runs = 0;
    world.system(Phase::Update, each([&runs](Position& pos, Velocity& vel) {
        runs += 1;
        pos.x += vel.x;
    }));
...
Test(system, each_supports_without_filters_across_multiple_tables) {
    ...
    world.system(Phase::Update, each<Without<Sleeping>>([&matchedEntities](Position& pos, Velocity& vel) {
        matchedEntities += 1;
        pos.x += vel.x;
    }));
...
Test(system, start_and_progress_run_phases_in_expected_order) {
    World world;
    std::vector<Phase> phases;

    world.system(Phase::PreStartup, markPhase(phases, Phase::PreStartup));
    world.system(Phase::Startup, markPhase(phases, Phase::Startup));
    world.system(Phase::PostStartup, markPhase(phases, Phase::PostStartup));
    world.system(Phase::PreUpdate, markPhase(phases, Phase::PreUpdate));
    world.system(Phase::Update, markPhase(phases, Phase::Update));
    world.system(Phase::PostUpdate, markPhase(phases, Phase::PostUpdate));
    world.system(Phase::Physics, markPhase(phases, Phase::Physics));
    world.system(Phase::PreRender, markPhase(phases, Phase::PreRender));
    world.system(Phase::Render, markPhase(phases, Phase::Render));

    world.start();
    world.progress();

    cr_assert_eq(phases.size(), 9);
    cr_assert_eq(phases[0], Phase::PreStartup);
    cr_assert_eq(phases[1], Phase::Startup);
    cr_assert_eq(phases[2], Phase::PostStartup);
    cr_assert_eq(phases[3], Phase::PreUpdate);
    cr_assert_eq(phases[4], Phase::Update);
    cr_assert_eq(phases[5], Phase::PostUpdate);
    cr_assert_eq(phases[6], Phase::Physics);
    cr_assert_eq(phases[7], Phase::PreRender);
    cr_assert_eq(phases[8], Phase::Render);
}
...
Test(system, systems_in_same_phase_run_in_registration_order) {
    ...
    world.system(Phase::Update, describeSystem(query<>(), [&calls](const EcsVec<TableId>&, World&) {
        calls.push_back(1);
    }));
    world.system(Phase::Update, describeSystem(query<>(), [&calls](const EcsVec<TableId>&, World&) {
        calls.push_back(2);
...
Test(system, each_handles_large_numbers_of_entities) {
    ...
    world.system(Phase::Update, each([&updatedCount, &updatedSum](Position& pos, Velocity& vel) {
        pos.x += vel.x;
        pos.y += vel.y;
        updatedCount += 1;
        updatedSum += pos.x;
...
Test(system, each_entity_exposes_the_matching_entity_to_the_callback) {
    ...
    world.system(Phase::Update, each([&visited](Entity entity, Position& pos, Velocity& vel) {
...
Test(system, each_entity_handles_large_numbers_of_entities) {
    ...
    int observedUpdated = 0;

    world.system(Phase::Update, each(
                     [&visited, &indexSum, &observedUpdated](Entity entity, Position& pos, Velocity& vel) {
                         visited += 1;
...
Test(system, each_supports_entity_components_and_without_combination) {
    ...
    world.system(Phase::Update, each<Without<Sleeping>>([&visited](Entity entity, Position& pos, Velocity& vel) {
        visited.push_back(entity);
        pos.x += vel.x;
    }));
...
Test(system, each_supports_entity_commands_and_without_combination) {
    ...
    world.system(Phase::Update, each<Without<Sleeping>>([](Entity entity, Position&, Commands& commands) {
...
Test(system, each_supports_const_component_params) {
    ...
    world.system(Phase::Update, each([&sum](const Position& pos, const Velocity& vel) {
...
Test(system, each_supports_from_world_row_params) {
    ...
    world.system(Phase::Update, each([&visited](RowEntity rowEntity, Position& pos, Velocity& vel) {
...
Test(system, each_supports_from_world_row_with_without_filters) {
    ...
    world.system(Phase::Update, each<Without<Sleeping>>([&visited](RowEntity rowEntity, Position& pos) {
        visited.push_back(rowEntity.value);
