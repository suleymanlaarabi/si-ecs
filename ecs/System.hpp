...
template <typename T>
concept HasFromWorldRow = requires(World& world, const Table& table, const EntityRow row) {
    T::fromWorldRow(world, table, row);
};

template <typename T>
concept HasQueryTerms = requires(Query& query) {
    T::addQueryTerms(query);
};
...
struct IsSupportedSystemParam {
    static constexpr bool value = std::same_as<Param, Entity> ||
...
struct SystemParamTraits {
    ...
    __attribute__((always_inline)) static inline decltype(auto) resolveTable(World& world, const Table& table) {
    ...
    __attribute__((always_inline)) static inline decltype(auto) resolveRow(Cache& cache, const uint32_t row) {
        if constexpr (std::same_as<Param, Entity>) {
            return cache[row];
        } else if constexpr (HasFromWorldTable<Param>) {
            return cache;
...
__attribute__((always_inline)) static inline void callRow(Func&& fn, CacheTuple& caches, const uint32_t row,
                                                          std::index_sequence<I...>) {
    fn(SystemParamTraits<std::tuple_element_t<I, ParamsTuple>>::resolveRow(std::get<I>(caches), row)...);
}

template <typename... Params>
struct ComponentsIter {
    template <typename Func>
    __attribute__((always_inline)) static inline void iter(Func&& func, const Table& table, World& world) {
        std::tuple<decltype(SystemParamTraits<Params>::resolveTable(world, table))...> caches(
            SystemParamTraits<Params>::resolveTable(world, table)...
        );

        for (uint32_t i = 0; i < table.size(); ++i) {
            callRow<std::tuple<Params...>>(func, caches, i, std::index_sequence_for<Params...>{});
        }
    }
};

template <typename... Terms, typename Func>
SystemDesc each(Func&& func) {
    ...
    static_assert(
        ...
        "Unsupported system param. Use T& or const T& for components, Entity for entity access, "
        "or implement fromWorldTable(World&, const Table&) / fromWorldRow(World&, const Table&, EntityRow)."
    );

    return {
        .query = buildSystemQuery<Args>(query<Terms...>(), tuple_index_sequence<Args>()),
        .callback = [func = std::forward<Func>(func)](
...
struct Related {
    ...
    static Related fromWorldRow(World& world, const Table& table, const EntityRow row) {
        const Entity parent = static_cast<RelationTarget<Relation>*>(table.getColumn(
            ComponentType::id<RelationTarget<Relation>>()))[row].target;
    ...
    template <typename T>
    T& get() {
        return *static_cast<T*>(this->targetTable.getComponent(this->targetRow, ComponentType::id<T>()));
    }
