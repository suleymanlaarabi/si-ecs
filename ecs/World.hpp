...
public:
    ...
    void addBatchComponents(const Entity entity, const ComponentId *cids, const uint32_t cidCount) {
        this->entityManager_.addBatchComponents(entity, cids, cidCount);
    }
    ...
    template<typename E>
    void unlisten(const EventId id) {
    ...
    template<typename E>
    void unlistenEntityEvent(const EntityEventId id) {
        // TODO: unlisten
    ...
    static SystemCondition condition(World &) {
        ...
        return {
            ...
            .check = +[](void *ctx, const World &world) {
                return std::bit_cast<Timer>(ctx).tick(world.deltaTime);
            }
        };
    }
};
